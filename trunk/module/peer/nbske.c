...
* Function:
*
*   This header provides routines for secret key encryption based on the
*   Advanced Encryption Standard (AES) selected by the National Institute of
*   Science and Technology (NIST).  This standard uses the Rijndael algorithm
*   for block encryption.  We have selected Cipher Block Chaining (CBC) as a
*   method of encrypting multiple block buffers prior to block encryption.
*
*   This is an implementation of the NodeBrain specification for secret key
*   encryption, and is an important component of NodeBrain communications.
*
* Specifications:
*
*                           Words    Bytes    Bits
*   Encryption block size       4       16     128
*   CBC cipher key              4       16     128
*   Rijndael cipher key       0-8     0-32   0-256
*
*   A variable length Rijndael cipher key is supported.  The algorithm
*   supports multiple encryption block sizes, but we have hard-coded a
*   block size of 128 bits.  A variable blocksize version was tested and
*   rejected because it performed at half the speed of this fixed blocksize
*   version.  (This code can be easily modified to support larger fixed
*   block sizes.  For performance reasons, multiple blocksizes should be
*   supported with multiple code blocks instead of loops.)
*   
*   
*      
*             Size        *
*   Words     1   2   3   4   5   6   7   8
*   Bytes     4   8  12  16  20  24  28  32
*   Bits     32  64  96 128 160 192 224 256
*
*   Rounds are a function of key and block size.  Keys sizes of 1, 2, and 3 may
*   be used for "less expensive" and "less secure" encryption.
*
*             Key         
*             1*  2   3   4   5   6   7   8
*           +------------------------------ 
*       B 1 | 1   2   3   7   8   9  10  11  
*       l 2 | 1   2   4   8   9  10  11  12  
*       o 3 | 1   3   5   9  10  11  12  13  
*       c 4 | 1   3   6  10  11  12  13  14 <- Currently supported block size
*       k 5 | 1   3   7  11  12  13  14
*         6 | 1   4   8  12  13  14
*         7 | 1   4   9  13  14
*         8 | 1   4  10  14                          
*
...
*   #include "nb.h"
*
*   skeKey(skeKEY *key,int keySize,unsigned int keyData[]);
*   skeCipher(unsigned int *buffer,unsigned int blocks,unsigned int *cipher,skeKEY *key);
*
*   skeSeedCipher(unsigned int cipher[4]);
*   skeRandCipher(unsigned int cipher[4]);
*   skeKeyData(unsigned int keySize,unsigned int keyData[]);
*   
* Description:
*
*   These routines support multi-block encryption/decryption.  The content of a block
*   after encryption is determined by multiple inputs.
*
*      1) The Rijndael cipher key. (4 to 32 bytes) 
*      2) The CBC cipher key. (16 bytes)
*      3) The content prior to encryption.
*      4) The content of other blocks in the same multi-block operation.  This is a
*         result of cipher block chaining (CBC) applied prior to block encryption.
*     
*   A key is prepared for use by calling skeKey.  The same key may be used for multiple
*   calls to skeCipher.  Each call to skeCipher encrypts or decrypts a multiple block
*   buffer.  
*     
*   key     - This is a pointer to an skeKEY structure allocated and used as
*             follows.
*
*                   skeKEY mykey;
*                   skeKey(&mykey,...);
*                   skeCipher(...,&mykey);
*
*   keySize - The absolute value of keySize specifies the number of words of key
*             data (keyData).  When this number is positive, the key will be
*             used for encryption.  When negative, the key will be used for
*             decryption.  When the keySize is zero, the skeCipher routine will 
*             return without modification to the buffer.  
*
*               0  -  Plain text option.
*               1  -  None reusable option.  This option provides the least security,
*                     but is fast.  It is intended only for situations where the
*                     CBC cipher key is modified for each use.  Otherwise it can
*                     be attacked with repeated use of known text.
*               2  -  Low security option.    
*               3  -  Medium security option.
*               4  -  High security option.
*              >4  -  Higher security options.
*                     
*             Only the first 60 words are used when a keySize greater than 60 is
*             specified.        
*
*   keyData - 0 to 8 words (32-bit unsigned integer words) of key data.  This is
*             the "secret key" for the Rijndael encryption that determines the
*             relationship between plain text and cipher text, provided the length
*             is greater than 0. 
* 
*   buffer  - Plain text to encrypt or cipher text to decrypt.  This buffer
*             should be allocated as an integer array to force alignment to a
*             a 32 bit word.  It should be cast as an unsigned char array for
*             use as a character buffer.  The encryption block size is 16 bytes
*             (128 bits), so the user must provide some method of handling
*             partial blocks.  For character data you can simply pad with one
*             or more null characters (x'00').  For binary data, if the length
*             is fixed and known, the value is unimportant, but the full block
*             must be retained for decryption.  For variable length binary data
*             a length must be recorded within the buffer, or somewhere else 
*             where it will be available after decryption.
*             
*   blocks  - Number of 128-bit (16 byte) blocks to encrypt/decrypt.  This is the
*             buffer size.  You must use the same size for encryption and decryption
*             because the content of an encryption block is influenced by the
*             content of other blocks in the buffer.
*
*   cipher  - This is a 128-bit CBC cipher key.  This key controls a simple
*             transformation of the buffer prior to application of the Rijndael
*             algorithm.  The contents of every encryption block in the buffer
*             contributes to this transformation, making brute-force attacks
*             on the block encryption transformation more difficult.  We use 128-bit
*             CBC encryption from left to right, and 32-bit CBC encryption right
*             to left.  The 32-bit cipher key is derived from the 128-bit cipher
*             key supplied.                         
*
*   Secret keys should be exchanged under the protection of public key encryption
*   (see nbpke.h).  For network communication, the CBC cipher key should be a
*   random value that changes for each multi-block operation following a
*   pseudo-random sequence known to the programs on both ends.  Used in this way,
*   it is similar to a non-reusable password scheme.  The initial random value
*   should be exchanged under the protection of public key encryption.
*     
*
* Credits:
*
*   On October 2, 2000, the Rijndael algorithm was announced as the winner of
*   a three year NIST sponsored competition for a new standard to replace DES.
*   Two cryptographers from Belgium, Joan Daemen and Vincent Rijmen, submit the
*   winning algorithm.   
*
*   This code is a revision of code downloaded from the National Institute
*   of Science and Technology Advanced Encryption Standard (AES) web site.
* 
*      http://csrc.nist.gov/encryption/aes
*
*   The heading from the original code is shown here.
*   
...
* Terminology:
*
*   We think of data buffers, blocks and expanded keys as vertical arrays
*   of 32 bit words.  The description provided by Daemen and Rijmen refers
*   to columns and rows of blocks and keys with a different orientation,
*   although they also comment on the orientation that we prefer.
*   
*     0123456789abcdef  {block as horizontal byte array}
*
*
*           048c        {state as described by Daemen and Rijmen}  
*           159d
*           26ae
*           37bf 
*
*           0  3
*           v  v
*       0 > 0123 <      {state as vertical word array - our orientation}
*           4567
*           89ab
*       3 > cdef < Rijndael column 3 [Our row 3]
*           ^  ^
*              Rijndael row 3 [Our column 3]
*
*
*   Daemen and Rijmen describe four invertible state transformation, two
*   of which we relabel to match our orientation.
*
*     Transform     Name using vertical array orientation
*     -----------   -------------------------------------- 
*     ByteSub     - byte substitution
*     ShiftRow    - roll column
*     MixColumn   - mix row 
*     AddRoundKey - add round key
* 
* Architecture Independence
*
*   When passing encrypted data over the network, data and keys must be
*   converted to and from network byte order.
*
*   When preparing a Rijndael key or CBC key.
*       int X = ntohl(char Y[4])
*   Before encrypting a buffer
*       buffer[i]=ntohl(buffer[i]); 
*   Before sending an encrypted buffer.
*       buffer[i]=htonl(buffer[i]);
*
*   After receiving an encrypted buffer.
*       buffer[i]=ntohl(buffer[i]);
*   After decrypting a buffer.
*       buffer[i]=htonl(buffer[i]);
*
*   o Consider building this into the encryption routines so encrypted
*     data is always in network byte order.
*   o Also consider using a last word as a checksum so calling routines
*     don't need to implement checksum validation.
*=============================================================================
* Change History:
*
*    Date    Name/Change
...
*                different.
*             2) Additional key sizes are supported.
...
int rounds[8]={0,1,3,6,10,11,12,13};
...
/*
*  Get next random CBC key
*
*    This is not intended as a good pseudo-random number generator.  For our
*    purposes, we just want a new CBC key.  Because of the way it 128 bit key
*    is used to sweep right by 16 bytes and sweep left by 4 bytes, only the
*    first word need change to impact every byte in a buffer.  It takes 4
...
/*
*  Initialize an skeKEY Structure - calculate round keys (key expansion)
*
*    keySize:  >0 Encrypt, <0 Decryption
...
void skeKey(skeKEY *key,int keySize,unsigned int keyData[]){
  ...
  if((key->mode=keySize)==0) return;        /* encryption is optional */
  if(keySize<0) keySize=-keySize;           /* length is absolute value */
  if(keySize>60) keySize=60;                /* truncate keys over the limit */
  if(keySize>7) key->rounds=14;             /* maximum rounds */
  else key->rounds=rounds[keySize];         /* set rounds based on key size */
  for(w=0;w<keySize;w++)                    /* copy key data to schedule */
    key->keySched[w]=keyData[w];
  wptrPrior=&key->keySched[0];              /* prior word pointer */
  wptr=&(key->keySched[keySize]);           /* current word pointer */
  rptr=rcon;                                /* address of round key constant */
  word=key->keySched[keySize-1];            /* start with last word of key */  
  for(;w<(4*(key->rounds+1));w++){
    if(w%keySize==0){
      word=(S[(word&0xff0000)>>16]^*rptr)<<24|S[(word&0xff00)>>8]<<16|S[word&0xff]<<8|S[(word&0xff000000)>>24];
      rptr++;
      }
    else if(keySize>6 && w%keySize==4){
      word=S[(word&0xff000000)>>24]<<24|S[(word&0xff0000)>>16]<<16|S[(word&0xff00)>>8]<<8|S[word&0xff];
      }  
    word^=*wptrPrior;  
    *wptr=word;
    wptr++;
  ...
  if(key->mode<0){ /* adjust for decryption, except first and last rounds */
    for(wptr-=5;wptr>=&key->keySched[4];wptr--){
      *wptr=U1[(*wptr&0xff000000)>>24]^U2[(*wptr&0xff0000)>>16]<<16^U3[(*wptr&0xff00)>>8]^U4[(*wptr&0xff)];
      }
    }    
  }
...
void skeCipher(unsigned int *buffer,unsigned int blocks,unsigned int cipher[4],skeKEY *key){
  ...
  unsigned int cipherWord;
  unsigned int *keyPtr;
  
  if(blocks<1) return;
  /* fold and mix 4-word cipher key into a 1-word cipher key */
  cipherWord=cipher[0]^cipher[1]^cipher[2]^cipher[3];
  cipherWord=T1[(cipherWord&0xff000000)>>24]^T2[(cipherWord&0xff0000)>>16]^T3[(cipherWord&0xff00)>>8]^T4[cipherWord&0xff];
  /* are we enciphering or deciphering */
  if(key->mode>0){          /* encryption */
    /* full block cipher from left to right */
    *(b+0)^=cipher[0];
    *(b+1)^=cipher[1];
    *(b+2)^=cipher[2];
    *(b+3)^=cipher[3];
    ...
    for(;b>=buffer;b-=4){
      /* one word cipher from right to left */     
      *(b+3)^=cipherWord; 
      *(b+2)^=*(b+3);
      *(b+1)^=*(b+2);
      *(b+0)^=*(b+1);
      ...
      for(r=0;r<rounds-1;r++){
        ...
        state[2]=*(b+2)^*(keyPtr++);
        state[3]=*(b+3)^*(keyPtr++);
        /* byte substitution, roll column, and mix row combined */
        *(b+0)=T1[(state[0]&0xff000000)>>24]^T2[(state[1]&0xff0000)>>16]^T3[(state[2]&0xff00)>>8]^T4[state[3]&0xff];
        *(b+1)=T1[(state[1]&0xff000000)>>24]^T2[(state[2]&0xff0000)>>16]^T3[(state[3]&0xff00)>>8]^T4[state[0]&0xff];
        *(b+2)=T1[(state[2]&0xff000000)>>24]^T2[(state[3]&0xff0000)>>16]^T3[(state[0]&0xff00)>>8]^T4[state[1]&0xff];
        *(b+3)=T1[(state[3]&0xff000000)>>24]^T2[(state[0]&0xff0000)>>16]^T3[(state[1]&0xff00)>>8]^T4[state[2]&0xff];
        }  
      /* key addition */ 
      state[0]=*(b+0)^*(keyPtr++);
      state[1]=*(b+1)^*(keyPtr++);
      state[2]=*(b+2)^*(keyPtr++);
      state[3]=*(b+3)^*(keyPtr++);
      /* byte substitution, roll column, and final key addition */
      *(b+0)=(((T1[(state[0]&0xff000000)>>24]&0x00ff0000)<<8)|
        (T1[(state[1]&0xff0000)>>16]&0x00ff0000)|
        ((T1[(state[2]&0xff00)>>8]&0x00ff0000)>>8)|
        ((T1[state[3]&0xff]&0x00ff0000)>>16))^*(keyPtr++); 
      *(b+1)=(((T1[(state[1]&0xff000000)>>24]&0x00ff0000)<<8)|
        (T1[(state[2]&0xff0000)>>16]&0x00ff0000)|
        ((T1[(state[3]&0xff00)>>8]&0x00ff0000)>>8)|
        ((T1[state[0]&0xff]&0x00ff0000)>>16))^*(keyPtr++);
      *(b+2)=(((T1[(state[2]&0xff000000)>>24]&0x00ff0000)<<8)|
        (T1[(state[3]&0xff0000)>>16]&0x00ff0000)|
        ((T1[(state[0]&0xff00)>>8]&0x00ff0000)>>8)|
        ((T1[state[1]&0xff]&0x00ff0000)>>16))^*(keyPtr++);
      *(b+3)=(((T1[(state[3]&0xff000000)>>24]&0x00ff0000)<<8)|
        (T1[(state[0]&0xff0000)>>16]&0x00ff0000)|
        ((T1[(state[1]&0xff00)>>8]&0x00ff0000)>>8)|
        ((T1[state[2]&0xff]&0x00ff0000)>>16))^*(keyPtr++);
      }
    }
  else if(key->mode<0){        /* decryption */
    for(;blocks>0;blocks--){
      /* Rijndael algorithm */	
      keyPtr=&(key->keySched[(rounds+1)*4]);
      for(r=rounds;r>1;r--){
        /* key addition */ 
        state[3]=*(b+3)^*(--keyPtr);
        state[2]=*(b+2)^*(--keyPtr);
        state[1]=*(b+1)^*(--keyPtr);
        state[0]=*(b+0)^*(--keyPtr);
        /* inverse byte substitution, roll column, and mix row combined */ 
        b[0]=T5[(state[0]&0xff000000)>>24]^T6[(state[3]&0xff0000)>>16]^T7[(state[2]&0xff00)>>8]^T8[state[1]&0xff];
        b[1]=T5[(state[1]&0xff000000)>>24]^T6[(state[0]&0xff0000)>>16]^T7[(state[3]&0xff00)>>8]^T8[state[2]&0xff];
        b[2]=T5[(state[2]&0xff000000)>>24]^T6[(state[1]&0xff0000)>>16]^T7[(state[0]&0xff00)>>8]^T8[state[3]&0xff];
        b[3]=T5[(state[3]&0xff000000)>>24]^T6[(state[2]&0xff0000)>>16]^T7[(state[1]&0xff00)>>8]^T8[state[0]&0xff];
        }
      /* key addition */  
      state[3]=*(b+3)^*(--keyPtr);
      state[2]=*(b+2)^*(--keyPtr);
      state[1]=*(b+1)^*(--keyPtr);
      state[0]=*(b+0)^*(--keyPtr);
      /* inverse byte substitution, roll column, and final key addition */
      *(b+3)=((((((S5[(state[3]&0xff000000)>>24]<<8)|S5[(state[2]&0xff0000)>>16])<<8)|S5[(state[1]&0xff00)>>8])<<8)|S5[state[0]&0xff])^*(--keyPtr);
      *(b+2)=((((((S5[(state[2]&0xff000000)>>24]<<8)|S5[(state[1]&0xff0000)>>16])<<8)|S5[(state[0]&0xff00)>>8])<<8)|S5[state[3]&0xff])^*(--keyPtr);
      *(b+1)=((((((S5[(state[1]&0xff000000)>>24]<<8)|S5[(state[0]&0xff0000)>>16])<<8)|S5[(state[3]&0xff00)>>8])<<8)|S5[state[2]&0xff])^*(--keyPtr);
      *(b+0)=((((((S5[(state[0]&0xff000000)>>24]<<8)|S5[(state[3]&0xff0000)>>16])<<8)|S5[(state[2]&0xff00)>>8])<<8)|S5[state[1]&0xff])^*(--keyPtr);
      /* one word cipher - right to left - undo */
      if(b>buffer) *(b-1)^=*b;
      *(b+0)^=*(b+1);
      *(b+1)^=*(b+2);
      *(b+2)^=*(b+3);
    ...
    *(b-1)^=cipherWord;   
    /* full block cipher - left to right - undo */     
    for(b-=8;b>=buffer;b-=4){
      *(b+4)^=*(b+0);
      *(b+5)^=*(b+1);
      *(b+6)^=*(b+2);
      *(b+7)^=*(b+3);
